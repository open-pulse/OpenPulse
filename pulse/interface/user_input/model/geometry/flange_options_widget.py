...
class FlangeOptionsWidget(QWidget):
    edited = pyqtSignal()

    def __init__(self, parent):
        super().__init__(parent)
        ui_path = UI_DIR / "model/geometry/valve_option_widget.ui"
        uic.loadUi(ui_path, self)

        self.pipeline = app().project.pipeline
        self.structure_type = Flange
        self.add_function = self.pipeline.add_flange
        self.attach_function = self.pipeline.connect_flanges
