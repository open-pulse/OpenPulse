...
class AcousticPressureInput(QDialog):
    def __init__(self, project, opv, *args, **kwargs):
        ...
        self.acoustic_pressure_tables_folder_path = get_new_path(self.acoustic_folder_path, "acoustic_pressure_tables")   
        ...
        self.list_acoustic_pressure_table_names = []
    ...
    def check_constant_values(self):
        ...
        if acoustic_pressure is not None:
            ...
            self.list_acoustic_pressure_table_names = self.get_acoustic_pressure_table_names_in_typed_nodes(self.nodes_typed)
            self.remove_acoustic_pressure_table_files(self.nodes_typed)
            ...
            message = "You must to inform at least one acoustic pressure to confirm the input!"
            PrintMessageInput([title, message, window_title])
    ...
    def load_table(self, lineEdit, header):
        ...
        self.project.create_folders_acoustic("acoustic_pressure_tables")
        self.new_load_path_table = get_new_path(self.acoustic_pressure_tables_folder_path, self.basename)
    ...
    def check_table_values(self):
        ...
        if self.lineEdit_load_table_path != "":
            if self.acoustic_pressure is not None:
                ...
                self.list_acoustic_pressure_table_names = self.get_acoustic_pressure_table_names_in_typed_nodes(self.nodes_typed)
                self.remove_acoustic_pressure_table_files(self.nodes_typed)
                self.project.set_acoustic_pressure_bc_by_node(self.nodes_typed, data, True)
                self.transform_points(self.nodes_typed)
        self.close()
    ...
    def get_acoustic_pressure_table_names_in_typed_nodes(self, list_node_ids):
        list_table_names = []
    ...
    def check_remove_bc_from_node(self):
        if self.lineEdit_nodeID.text() != "":
            ...
            if node in self.preprocessor.nodes_with_acoustic_pressure:
                ...
                self.remove_acoustic_pressure_table_files([picked_node_id])
                self.preprocessor.set_acoustic_pressure_bc_by_node(picked_node_id, [None, None])
                self.transform_points(picked_node_id)
                self.load_nodes_info()
                # self.close()

    def remove_acoustic_pressure_table_files(self, list_node_ids):
        str_key = "acoustic pressure"
        folder_table_name = "acoustic_pressure_tables"
        for node_id in list_node_ids:
            node = self.preprocessor.nodes[node_id]
            if node.acoustic_pressure_table_name is not None:
                table_name = node.acoustic_pressure_table_name
            ...
            elif len(self.list_acoustic_pressure_table_names) > 0:
                for table_name in self.list_acoustic_pressure_table_names:
                    if self.project.file.check_if_table_can_be_removed_in_acoustic_model(   node_id, 
                                                                                            str_key, 
                                                                                            table_name,
                                                                                            folder_table_name   ):
    ...
    def confirm_table_file_removal(self, table_name, label):
        if table_name is not None:
            ...
            if read._continue:
                self.project.remove_acoustic_table_files_from_folder(table_name, "acoustic_pressure_tables")
        ...
        if len(self.preprocessor.nodes_with_acoustic_pressure)>0:
            ...
            if read._continue:
                for node in _nodes_with_acoustic_pressure:
                    ...
                    self.remove_acoustic_pressure_table_files([node_id])
