...
def results_plot(   coordinates, connectivity, Uxyz, **kwargs): 

    scf = kwargs.get("scf", 0.4)
    freq_n = kwargs.get("freq_n", None)
    deformation = kwargs.get("deformation", True)
    Show_nodes = kwargs.get("Show_nodes", True)
    Undeformed = kwargs.get("Undeformed", False)
    Deformed = kwargs.get("Deformed", True)
    Animate_Mode = kwargs.get("Animate_Mode", True)
    ...
    if Uxyz.any():
        ...
        if deformation:
            ...
            Undeformed, Deformed, Save = False, True, False
        ...
        norm = plt.Normalize(min(r), max(r),clip=False)
        ...
        for start, end in connectivity:
            ...
            if deformation:
                ...
                segments_u = np.zeros_like(segments_p)
        ...
        if Animate_Mode:
            ...
            def animate(i):
                ...
                if deformation:
                    ...
                    r_i = r_m*np.cos(2 * np.pi * (i*4/frames)*t) 
                ...
                if Show_nodes:
                    ...
                    if deformation:
                        ...
                        r_ii = r*np.cos(2 * np.pi * (i*4/frames)*t)
